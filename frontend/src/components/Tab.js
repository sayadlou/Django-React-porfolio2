...
const Tab = ({homeAboutTab}) => {
    ...
    return (
        <div>
            <span onClick={action1} style={{cursor: "pointer"}}
                  className={`d-inline m-1 f14 ${p1Class}`}>{` ${homeAboutTab[0].title} `}</span>
            <span onClick={action2} style={{cursor: "pointer"}}
                  className={`d-inline m-1 f14 ${p2Class}`}>{` ${homeAboutTab[1].title} `}</span>
            <span onClick={action3} style={{cursor: "pointer"}}
                  className={`d-inline m-1 f14 ${p3Class}`}>{` ${homeAboutTab[2].title} `}</span>
            <br/>
            <p className='mt-3 f14'>{description}</p>
            <Link className="float-left text-fosfori f14" href="">
                <span style={{color: "#0098A0", textDecoration: "underline"}}>
                               مطالعه بیشتر
                </span>
            </Link>
        </div>
